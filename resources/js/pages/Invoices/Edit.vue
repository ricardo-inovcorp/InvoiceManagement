...
const form = useForm({
    ...
    issue_date: formatDateForInput(props.invoice.issue_date),
    ...
    payment_date: formatDateForInput(props.invoice.payment_date),
    file: null,
...
onMounted(() => {
    if (props.invoice.items && props.invoice.items.length > 0) {
        ...
        items.value = props.invoice.items.map(item => ({
            id: item.id,
